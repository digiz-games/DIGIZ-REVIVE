...
///1920, 1200
  ...
  for (var i in enemies2) {
    ...
    try{
      game.physics.arcade.moveToObject(enemies2[i], player, 150);
  ...
  for (var i in enemies3) {
    ...
    try{
      game.physics.arcade.moveToObject(enemies3[i], player, 200);
  ...
  for (var i in enemies){
      ...
      game.physics.arcade.collide(enemies[i], asteroides2);
      game.physics.arcade.collide(enemies[i], asteroides3);
  ...
  for (var i in enemies){
      ...
      game.physics.arcade.collide(player, asteroides2,function(){vida -= 0.5});
      game.physics.arcade.collide(player, asteroides3,function(){vida -= 0.5});
...
fireenemy1: function(){
  ...
  for (var i = 0; i < game.rnd.integerInRange(1, enemiesweapon.length); i++ ) {
    try{
      ...
      enemiesweapon[randomx].onFire.add(this.fireenemy1sound);
  ...
  fireenemy2: function(){
    ...
    for (var i = 0; i < game.rnd.integerInRange(1, enemiesweapon2.length); i++ ) {
      try{
        ...
        enemiesweapon2[randomx].onFire.add(this.fireenemy2sound);
    ...
    for (var i = 0; i < game.rnd.integerInRange(1, enemiesweapon3.length); i++ ) {
      try{
        ...
        enemiesweapon3[randomx].fire();
          enemiesweapon3[randomx].onFire.add(this.fireenemy3sound);
...
colisionenemy1:function(enemy,bullet){
  ...
  enemy.animations.currentAnim.onComplete.add(function () {	enemy.kill(); enemy.reset(game.rnd.integerInRange(0, 2000),-10); enemy.frame = 0;}, this);

/*
  var index = enemies.indexOf(enemy.id);
  if (index > -1) {
  enemies.splice(index, 1);
  enemiesweapon.splice(index, 1);
  }
*/
...
enemy.kill(); enemy.reset(game.rnd.integerInRange(0, 2000),-10); enemy.frame = 0;
/*
      enemy.destroy();
      var index = enemies2.indexOf(enemy.id);
      if (index > -1) {
      enemies2.splice(index, 1);
      enemiesweapon2.splice(index, 1);
      }
...
enemy.kill(); enemy.reset(game.rnd.integerInRange(0, 2000),2010); enemy.frame = 0;
/*
      enemy.destroy();
      var index = enemies3.indexOf(enemy.id);
      if (index > -1) {
      enemies3.splice(index, 1);
      enemiesweapon3.splice(index, 1);
      }
*/
  }, this);
...
dead: function(){
  ...
  enemies3 = [];
  enemiesweapon4 = [];
  player.animations.currentAnim.onComplete.add(function () {	this.state.start('Dead',true,false);}, this);
},
